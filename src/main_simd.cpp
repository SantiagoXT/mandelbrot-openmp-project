...
void mergeChannels(
    ...
    #pragma omp parallel for schedule(static)
    for (std::size_t i = 0; i < image.size(); ++i) {
        image[i] = Pixel{red[i], green[i], blue[i]};
    }
}

// ===============================
// Tarea B: Filtro de convolucion 2D con estructura SPMD + SIMD
// La imagen se separa en canales para que el bucle mas interno tenga accesos contiguos.
// ===============================
void applyBoxBlur(
    ...
    std::vector<std::uint8_t> inputR(totalPixels);
    std::vector<std::uint8_t> inputG(totalPixels);
    std::vector<std::uint8_t> inputB(totalPixels);

    std::vector<std::uint8_t> outputR(totalPixels);
    std::vector<std::uint8_t> outputG(totalPixels);
    std::vector<std::uint8_t> outputB(totalPixels);

    splitChannels(input, inputR, inputG, inputB);

    int kernelSize = 2 * radius + 1;
    int kernelArea = kernelSize * kernelSize;

    // Region paralela estilo SPMD: todos los hilos ejecutan la misma region
    // y el trabajo se reparte con omp for.
    #pragma omp parallel
    {
            ...
            for (int x = 0; x < width; ++x) {
                ...
                if (isInterior) {
                    for (int ky = -radius; ky <= radius; ++ky) {
                        std::size_t rowBase = static_cast<std::size_t>(y + ky) * static_cast<std::size_t>(width)
                                            + static_cast<std::size_t>(x - radius);

                        // Bucle mas interno vectorizado.
                        #pragma omp simd reduction(+:sumR,sumG,sumB)
                        for (int k = 0; k < kernelSize; ++k) {
                            std::size_t index = rowBase + static_cast<std::size_t>(k);
                    ...
                    // Bordes: version escalar con clamp para mantener el mismo comportamiento.
                    for (int ky = -radius; ky <= radius; ++ky) {
                        int sampleY = clampInt(y + ky, 0, height - 1);

                        for (int kx = -radius; kx <= radius; ++kx) {
                            int sampleX = clampInt(x + kx, 0, width - 1);
                            std::size_t index = static_cast<std::size_t>(sampleY) * static_cast<std::size_t>(width)
                ...
                std::size_t outputIndex = static_cast<std::size_t>(y) * static_cast<std::size_t>(width)
                                        + static_cast<std::size_t>(x);

                outputR[outputIndex] = static_cast<std::uint8_t>(sumR / kernelArea);
                outputG[outputIndex] = static_cast<std::uint8_t>(sumG / kernelArea);
                outputB[outputIndex] = static_cast<std::uint8_t>(sumB / kernelArea);
            }
        }
    }

    mergeChannels(output, outputR, outputG, outputB);
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== Mandelbrot OpenMP SIMD convolution benchmark ===" << std::endl;
